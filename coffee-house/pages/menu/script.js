...
window.addEventListener(
  ...
  function (event) {
    if (document.querySelector('body').offsetWidth + isHasScroll() > 768) {
      ...
      document.querySelector('body').offsetWidth + isHasScroll() <= 768 &&
...
navMenu[navMenu.length - 1].addEventListener('click', (event) =>
  openOrCloseBurgerMenu()
);
...
function activateBtns(element) {
  menuTabs.forEach((element) => {
    ...
    element
      .querySelector('.tab-item-icon')
      .classList.remove('tab-item-icon-active');
    element
      .querySelector('.tab-item-text')
      .classList.remove('tab-item-text-active');
  });
  ...
  data.forEach((element, index) => {
    if (element.category === category) {
      ...
      document.querySelector(
        `.item-box-${num}`
      ).style.backgroundImage = `url(../../assets/images/menu/${category}/${category}-${num}.png)`;
      num++;
...
function openModalWindow(element, index) {
            ...
            <div class="modal-size">
                ...
                <div class="size-btns">
                    <div class="size-btn btn-active" id="s">
                        ...
                        <p class="size-btn-text btn-text-active">${
  ...
  document.querySelector(
    '.modal-img'
  ).style.backgroundImage = `url(../../assets/images/menu/${element.category}/${element.category}-${num}.png)`;
  ...
  sizeBtns.forEach((element1) => {
    element1.addEventListener('click', (event) => {
      sizeBtns.forEach((element2) => {
        ...
        element2
          .querySelector('.size-btn-circle')
          .classList.remove('btn-circle-active');
        element2
          .querySelector('.size-btn-circle-text')
          .classList.remove('btn-circle-text-active');
        element2
          .querySelector('.size-btn-text')
          .classList.remove('btn-text-active');
      });
      element1.classList.add('btn-active');
      element1
        .querySelector('.size-btn-circle')
        .classList.add('btn-circle-active');
      element1
        .querySelector('.size-btn-circle-text')
        .classList.add('btn-circle-text-active');
      element1.querySelector('.size-btn-text').classList.add('btn-text-active');
      sizeCost =
        Number(element.price) + Number(element.sizes[element1.id]['add-price']);
      document.querySelector('.modal-total-sum').innerHTML = `$${(
        sizeCost + addsCost
      ).toFixed(2)}`;
    });
  });
  ...
  addBtns.forEach((element1) => {
    element1.addEventListener('click', (event) => {
      if (element1.classList.contains('add-active')) {
        ...
        element1
          .querySelector('.additives-btn-circle')
          .classList.remove('add-circle-active');
        element1
          .querySelector('.additives-btn-circle-text')
          .classList.remove('add-circle-text-active');
        element1
          .querySelector('.additives-btn-text')
          .classList.remove('add-text-active');
        addsCost =
          addsCost - Number(element.additives[element1.id]['add-price']);
        document.querySelector('.modal-total-sum').innerHTML = `$${(
          sizeCost + addsCost
        ).toFixed(2)}`;
      } else {
        ...
        element1
          .querySelector('.additives-btn-circle')
          .classList.add('add-circle-active');
        element1
          .querySelector('.additives-btn-circle-text')
          .classList.add('add-circle-text-active');
        element1
          .querySelector('.additives-btn-text')
          .classList.add('add-text-active');
        addsCost =
          addsCost + Number(element.additives[element1.id]['add-price']);
        document.querySelector('.modal-total-sum').innerHTML = `$${(
          sizeCost + addsCost
